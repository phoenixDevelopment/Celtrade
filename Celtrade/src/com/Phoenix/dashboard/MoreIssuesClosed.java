...
/**
 * Servlet implementation class MoreIssuesClosed
 */
@WebServlet("/MoreIssuesClosed.do")
public class MoreIssuesClosed extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getSession().setAttribute("depIDClosed", this.depId);
		request.getSession().setAttribute("startPageClosed", this.startPage);
		request.getSession().setAttribute("issueListClosed", this.openIssuesDep);
		request.getSession().setAttribute("totalPagesClosed", totalPages);
	...
	private int getTotalPages(int depid){
		int total = 0;
		try {
			PreparedStatement stmt = conn.prepareStatement("select count(*) as 'total' from issuesClosed where iddepartment=?");
			stmt.setInt(1, depid);
	...
	private ArrayList<OpenIssue> getAllIssues(int depid){
		...
		try {
			PreparedStatement stmt = conn.prepareStatement("select * from issuesClosed where iddepartment=? order by closedAt asc limit ?,?");
			stmt.setInt(1, depid);
			stmt.setInt(2, startPage);
			stmt.setInt(3, (startPage+PERPAGE));
			ResultSet res = stmt.executeQuery();
			while(res.next()){
				openIssues.add(new OpenIssue(res.getString("username"),res.getString("depName"),res.getInt("idIssues"), res.getString("issueName"), res.getString("issuetype"), res.getString("priority"), res.getString("type"),openSince(res.getInt("closedAt"))));			
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
