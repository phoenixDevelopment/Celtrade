...
/**
 * Servlet implementation class ViewWorkOrders
 */
@WebServlet("/ViewWorkOrders.do")
public class ViewWorkOrders extends HttpServlet {
	...
	private int getWorkOrdersView(){
		...
		try {
			...
			if(res.next()){
				...
				out.println("</div>");
				status = updateWorkOrders(res.getTimestamp("time"));
			}
	...
	private int updateWorkOrders(Timestamp time){
		int status =0;
		try {
			PreparedStatement stmt = conn.prepareStatement("call updateWorkOrders(?)");
